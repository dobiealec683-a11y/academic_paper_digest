...
def _run_notebooklm_command(cmd: list[str]) -> subprocess.CompletedProcess[str]:
    try:
        return subprocess.run(cmd, capture_output=True, text=True, check=True)
    except FileNotFoundError as exc:
        raise RuntimeError(
            "The `notebooklm` CLI was not found. Install it and make sure it is on PATH."
        ) from exc
...
class NotebookLMBridge:
    ...
    def create_notebook(self, topic_name: str) -> str:
        ...
        if self.mode == "enterprise":
            ...
            cmd = ["notebooklm", "create", "--use", "--json", topic_name]
            result = _run_notebooklm_command(cmd)
            output = result.stdout.strip()
            notebook_id = _extract_notebook_id(output)
            logger.info(
                "Created notebook through notebooklm CLI",
                extra=log_context(output=output, notebook_id=notebook_id),
            )
            return notebook_id
        else:
    ...
    def upload_file_source(self, notebook_id: str, file_path: str | Path) -> bool:
        ...
        if self.mode == "enterprise":
            ...
            cmd = ["notebooklm", "source", "add", "--type", "file"]
            if notebook_id:
                cmd.extend(["--notebook", notebook_id])
            cmd.append(str(file_path))
            _run_notebooklm_command(cmd)
            return True
        else:
            return True
    ...
    def add_web_source(self, notebook_id: str, url: str) -> bool:
        ...
        if self.mode == "enterprise":
            ...
            cmd = ["notebooklm", "source", "add", "--type", "url"]
            if notebook_id:
                cmd.extend(["--notebook", notebook_id])
            cmd.append(url)
            _run_notebooklm_command(cmd)
    ...
    def ask_notebook(self, notebook_id: str, prompt: str, output_filename: str | None = None) -> str:
        ...
        if self.mode == "enterprise":
            ...
            cmd = ["notebooklm", "ask", "--timeout", "120"]
            if notebook_id:
                cmd.extend(["--notebook", notebook_id])
            cmd.append(prompt)
            result = _run_notebooklm_command(cmd)
